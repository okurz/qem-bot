...
def test_make_submission_from_gitea_pr_dry(mocker: MockerFixture) -> None:
    ...
    mocker.patch("openqabot.loader.gitea._read_json", return_value=[])

    def mock_add_comments(incident: dict, _comments: list, *, dry: bool) -> None:
        incident["channels"] = [1]

    mocker.patch("openqabot.loader.gitea._add_comments_and_referenced_build_results", side_effect=mock_add_comments)

    def mock_add_packages(incident: dict, _token: dict, _files: list, *, dry: bool) -> None:
        incident["packages"] = ["pkg"]

    mocker.patch("openqabot.loader.gitea._add_packages_from_files", side_effect=mock_add_packages)
...
def test_make_submission_from_gitea_pr_skips(mocker: MockerFixture, caplog: pytest.LogCaptureFixture) -> None:
    ...
    caplog.clear()
    mocker.patch("openqabot.loader.gitea._is_build_acceptable_and_log_if_not", return_value=False)
    ...
    caplog.clear()
    mocker.patch("openqabot.loader.gitea._is_build_acceptable_and_log_if_not", return_value=True)
    res = gitea.make_submission_from_gitea_pr(pr, {}, only_successful_builds=False, only_requested_prs=False, dry=False)
    assert res is None
...
def test_make_submission_from_gitea_pr_dry_other_number_passes(mocker: MockerFixture) -> None:
    ...
    mocker.patch("openqabot.loader.gitea._add_reviews", return_value=1)

    def mock_add_chan(inc: dict, *_: Any, **__: Any) -> None:
        inc["channels"].append("chan")

    mocker.patch("openqabot.loader.gitea._add_comments_and_referenced_build_results", side_effect=mock_add_chan)

    def mock_add_pkg(inc: dict, *_: Any, **__: Any) -> None:
        inc["packages"].append("pkg")

    mocker.patch("openqabot.loader.gitea._add_packages_from_files", side_effect=mock_add_pkg)
...
def test_make_submission_from_gitea_pr_no_reviews(mocker: MockerFixture, caplog: pytest.LogCaptureFixture) -> None:
    ...
    pr = {"number": 123, "state": "open", "url": "url", "base": {"repo": {"full_name": "owner/repo", "name": "repo"}}}
    mocker.patch("openqabot.loader.gitea._get_json", return_value=[])
    mocker.patch("openqabot.loader.gitea._add_reviews", return_value=0)
    res = gitea.make_submission_from_gitea_pr(pr, {}, only_successful_builds=False, only_requested_prs=True, dry=False)
    assert res is None
    assert "PR git:123 skipped: No reviews" in caplog.text
...
def test_make_submission_from_gitea_pr_exception(caplog: pytest.LogCaptureFixture) -> None:
    ...
    res = gitea.make_submission_from_gitea_pr(pr, {}, only_successful_builds=False, only_requested_prs=False, dry=False)
    assert res is None
    assert "PR git:123 processing failed" in caplog.text


def test_is_review_requested_by_explicit_users() -> None:
    review = {"user": {"login": "user1"}}
    assert gitea._is_review_requested_by(review, users=("user1",))
    assert not gitea._is_review_requested_by(review, users=("user2",))
