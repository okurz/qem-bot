...
def test_get_open_prs_returns_empty_on_dry_run(mocker: MockerFixture) -> None:
    mocker.patch("openqabot.loader.gitea._read_json", return_value=42)
...
def test_get_open_prs_returns_specified_pr(mocker: MockerFixture) -> None:
    mocked_get_json = mocker.patch("openqabot.loader.gitea._get_json", return_value=42)
...
def test_get_open_prs_metadata_error(mocker: MockerFixture, caplog: pytest.LogCaptureFixture) -> None:
    ...
    mocker.patch("openqabot.loader.gitea._get_json", side_effect=requests.RequestException("error"))
    res = gitea.get_open_prs({}, "repo", dry=False, number=124)
    assert res == []


def test_get_open_prs_iter_pages(mocker: MockerFixture) -> None:
    mocker.patch("openqabot.loader.gitea._get_json", side_effect=[[1], [2], []])
...
def test_get_open_prs_json_error(mocker: MockerFixture, caplog: pytest.LogCaptureFixture) -> None:
    ...
    mocker.patch("openqabot.loader.gitea._get_json", side_effect=requests.exceptions.JSONDecodeError("msg", "doc", 0))
    res = gitea.get_open_prs({}, "repo", dry=False, number=None)
    assert res == []
    assert "Fetching open PRs failed" in caplog.text


def test_get_open_prs_request_error(mocker: MockerFixture, caplog: pytest.LogCaptureFixture) -> None:
    caplog.set_level(logging.ERROR, logger="bot.loader.gitea")
    mocker.patch("openqabot.loader.gitea._get_json", side_effect=requests.exceptions.RequestException("error"))
    res = gitea.get_open_prs({}, "repo", dry=False, number=None)
    assert res == []
    assert "Fetching open PRs failed" in caplog.text
...
def test_get_open_prs_specific_number_key_error(mocker: MockerFixture, caplog: pytest.LogCaptureFixture) -> None:
    ...
    caplog.set_level(logging.WARNING, logger="bot.loader.gitea")
    mocker.patch("openqabot.loader.gitea._get_json", side_effect=KeyError("missing_field"))
