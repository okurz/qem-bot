...
def test_add_build_result_inconsistent_scminfo(mocker: MockerFixture, caplog: pytest.LogCaptureFixture) -> None:
    ...
    mocker.patch("openqabot.loader.gitea._add_channel_for_build_result")
    gitea._add_build_result(incident, res, BuildResults())
    assert "Inconsistent SCM info for project" in caplog.text
...
def test_is_build_result_relevant_repo_mismatch(mocker: MockerFixture) -> None:
    ...
    assert not gitea._is_build_result_relevant(res, {"x86_64"})


def test_add_build_results_url_mismatch_just_passes() -> None:
    gitea._add_build_results({}, ["http://nomatch.com"], dry=True)
...
def test_add_build_results_http_error(mocker: MockerFixture, caplog: pytest.LogCaptureFixture) -> None:
    ...
    incident = {"number": 123}
    gitea._add_build_results(incident, ["http://obs/project/show/proj"], dry=False)
    assert "Build results for project proj unreadable" in caplog.text
...
def test_is_build_result_relevant_arch_filter(mocker: MockerFixture) -> None:
    ...
    res = {"arch": "s390x"}
    assert not gitea._is_build_result_relevant(res, {"x86_64"})
    assert gitea._is_build_result_relevant(res, {"s390x"})
...
def test_add_build_result_published(mocker: MockerFixture) -> None:
    ...
    mocker.patch("openqabot.loader.gitea._add_channel_for_build_result", return_value="chan")
    incident = {}
    res = mocker.Mock()
    res.findall.return_value = []
    res.get.side_effect = lambda k: "other" if k == "state" else "val"
    results = BuildResults()
    gitea._add_build_result(incident, res, results)
...
def test_is_build_result_relevant_repo_match(mocker: MockerFixture) -> None:
    ...
    assert gitea._is_build_result_relevant(res, {"x86_64"})
...
def test_add_build_results_dry_124(mocker: MockerFixture) -> None:
    ...
    incident = {"number": 124}
    gitea._add_build_results(incident, ["http://obs/project/show/proj"], dry=True)
...
def test_add_build_results_duplicate_channels(mocker: MockerFixture) -> None:
    ...
    mocker.patch("openqabot.loader.gitea._add_channel_for_build_result", side_effect=mock_add_channel)
    mocker.patch("openqabot.config.settings.obs_products", "SLES")
    ...
    gitea._add_build_results(submission, ["http://obs/project/show/proj"], dry=False)
...
def test_add_comments_bot_comments_no_urls(mocker: MockerFixture, caplog: pytest.LogCaptureFixture) -> None:
    ...
    gitea._add_comments_and_referenced_build_results(submission, comments, dry=True)
    mock_add_build_results.assert_not_called()
    assert "No OBS URLs found in comments" in caplog.text
