...
class IncrementConfig:
    ...
    def from_config_entry(cls, entry: dict[str, Any]) -> IncrementConfig:
        ...
        return cls(
            ...
            project_base=entry.get("project_base", "SUSE:SLFO:Products:SLES:16.0"),
            build_project_suffix=entry.get("build_project_suffix", "TEST"),
            diff_project_suffix=entry.get("diff_project_suffix", "PUBLISH/product"),
            build_listing_sub_path=entry.get("build_listing_sub_path", "product"),
            build_regex=entry.get("build_regex", config.BUILD_REGEX),
            product_regex=entry.get("product_regex", "^SLE.*"),
